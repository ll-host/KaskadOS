...
setupLogfile()
{
    ...
    {
        QFile source( logFile().toLocal8Bit() );
        if ( source.open( QIODevice::ReadOnly | QIODevice::Text ) )
        {
            const QByteArray tail = source.readAll().right( LOGFILE_SIZE - ( LOGFILE_SIZE / 4 ) );
            source.close();

            QSaveFile destination( logFile().toLocal8Bit() );
            if ( destination.open( QIODevice::WriteOnly | QIODevice::Text ) )
            {
                if ( destination.write( tail ) != tail.size() || !destination.commit() )
                {
...
operator<<( QDebug& s, const RedactedCommand& l )
{
    ...
    {
        if ( item == "usermod" || item.endsWith( "/usermod" ) )
        {
            isUsermod = true;
            break;
        }
