...
void
LibCalamaresTests::testRedactedCommand()
{
    const QString yescryptHash = QStringLiteral( "$y$j9T$salt$secret" );
    QString output;
    {
        QDebug debug( &output );
        debug << Logger::RedactedCommand( { "/usr/bin/usermod", "-p", yescryptHash, "alice" } );
    }

    QVERIFY( !output.contains( yescryptHash ) );
    QVERIFY( output.contains( QStringLiteral( "<password>" ) ) );
    QVERIFY( output.contains( QStringLiteral( "alice" ) ) );

    output.clear();
    {
        QDebug debug( &output );
        debug << Logger::RedactedCommand(
            { "usermod", QStringLiteral( "--password=" ) + yescryptHash, "root" } );
    }

    QVERIFY( !output.contains( yescryptHash ) );
    QVERIFY( output.contains( QStringLiteral( "--password=<password>" ) ) );
}
